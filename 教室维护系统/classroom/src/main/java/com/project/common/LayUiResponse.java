...
public class LayUiResponse<T> implements Serializable {
    ...
    private LayUiResponse(int code, List<T> data,int total){
        this.code = code;
        this.data = data;
        this.total=total;
    }
    ...
    public static <T> LayUiResponse<T> createBySuccess(List<T> data,int total){
        return new LayUiResponse<T>(com.project.common.ResponseCode.SUCCESS.getCode(),data,total);
    }
